...
Prep rm -fr generated widgets.json

1. Introduce the cdaysdemo repository - list the files in the folder
2. Mention installing go and envoy, run install_protoc, emacs proto/demo.proto, talk through it.
3. run generate.sh and show the generated folder, cd generated && find *
4. run_grpc.go - introduce kreya (postman or insomnia) - get the index page in grpc in Kreya
5. run_envoy.go - helloworld show page in kreya then (switch to browser) to prove it comes back as a page.
6. emacs envoy-config.yaml
7. back to kreya rest - move on to widgets - classic JSON crud cycle - create a widget
8. Show what happens when try to create short name widget - validation?
9. Show we can use transcoding for anything, eg. a file 
10. Then to streaming - showing the Envoy log as the big file is downloaded and stuck back together.
